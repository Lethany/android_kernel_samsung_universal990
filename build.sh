...
export VARIANT=eur
export ARCH=arm64
export BUILD_CROSS_COMPILE=/home/lee/Android/Toolchains/aarch64-linux-android-4.9/bin/aarch64-linux-android-
...
z3sxxx)
	case $VARIANT in
	can|duos|eur|xx)
		KERNEL_DEFCONFIG=exynos9830-z3sxxx_nemesis_defconfig
		;;
	*)
...
FUNC_BUILD_RAMDISK()
{
	...
	z3sxxx)
		case $VARIANT in
		can|duos|eur|xx)
			rm -f $RDIR/ramdisk/G988/split_img/boot.img-zImage
			mv -f $RDIR/arch/$ARCH/boot/boot.img-zImage $RDIR/ramdisk/G988/split_img/boot.img-zImage
			cd $RDIR/ramdisk/G988
			./repackimg.sh --nosudo
...
FUNC_BUILD_ZIP()
{
	...
	z3sxxx)
		case $VARIANT in
		can|duos|eur|xx)
			mv -f $RDIR/ramdisk/G988/image-new.img $RDIR/build/$MODEL-$VARIANT.img
			;;
		*)
			echo "Unknown variant: $VARIANT"
			exit 1
